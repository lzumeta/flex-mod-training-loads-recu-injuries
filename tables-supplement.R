...
simsummary_avg_df <- data.frame(data_generation1 = vector("character", 0L),
                                ...
                                data_generation4 = vector("character", 0L),
...
for (h in seq_along(h_labels)) {
  for (ll in l) {
    ...
    h_label <- h_labels[[h]]
    for (i in seq_along(name_true_sigmas)) {   ## sigmas
      name_true_sigma <- name_true_sigmas[[i]]
      true_sigma      <- true_sigmas[[i]]
      for (j in seq_along(model_names)) { ## models
        ...
        sim_res_aux <- data.frame(data_generation1 = paste0("hshape*ztz ", h),
                                  data_generation2 = paste0("L = ", ll),
                                  data_generation3 = paste0("$\\sigma_b$ = ", true_sigma),
                                  data_generation4 = paste0("model: ", model_label))
...
tab <- simsummary_avg_df |> 
  mutate(data_generation4 = case_when(data_generation4 == "model: Uncons." ~ "\\textit{Uncons.}",
                                      data_generation4 == "model: Constr." ~ "\\textit{Constr.}",
                                      data_generation4 == "model: Ridge" ~ "\\textit{Ridge}"
  )) |> 
  kbl(booktabs = T, align = c("l", "l", "l", "l", "c", "c", "c", "c"),
      ...
      col.names = c("WCE shape", "L players", "Heterogeneity", " ", "$h_{t, t_z, z(t_z)}$", "$\\sigma_b$", "$h_{t, t_z, z(t_z)}$"),
      format = "latex", digits = 3, escape = FALSE) |> 
  kable_styling() |> 
  add_header_above(c("Data generation mechanism" = 3, "Model" = 1, "Mean RMSE" = 2, "Mean Coverage" = 1)) |> 
  # row_spec(0, bold = TRUE) |> 
  collapse_rows(columns = c(1:2), latex_hline = "none", valign = "middle") 
  ...
  for (h in seq_along(h_labels)) {
    ...
    for (i in seq_along(name_true_sigmas)) {   ## sigmas
      ...
      for (j in seq_along(model_names)) { ## models
        ...
        reduced_res_aux <- data.frame(data_generation1 = paste0("L = ", ll, " players"),
                                      data_generation2 = paste0("shape  ", h),
                                      data_generation3 = paste0("$\\sigma_b$ = ", true_sigma))
        ...
        if (length(models_aic_df)) {
          models_aic_df <- left_join(models_aic_df, reduced_res,
                                     by = c("job.id", "data_generation1", "data_generation2", "data_generation3"))
        } else {
...
## LaTeX table with simulation results 
tab <- aic_df |> 
  select(1,2,3, 4,6,5, 7,9,8, 10,12,11) |>
  ...
  kbl(booktabs = T, align = c("l", "l", "l", "c", "c", "c", "c", "c", "c", "c", "c", "c"),
      ...
      col.names = c("L players", "WCE shape","Heterogeneity",
      ...
      format = "latex", digits = 3, escape = FALSE) |> 
  kable_styling() |> 
  add_header_above(c("Data generation mechanism" = 3, " " = 7)) |> 
